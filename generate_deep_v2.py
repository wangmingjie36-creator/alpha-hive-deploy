...
# pattern 1/5: 外部文本消毒 + persona 从 MD 加载（防御式导入：VM 同步竞态下回退到默认行为）
try:
        ...
        try:
            ...
            if news:
                ...
                for n in news:
                    ...
                    # pattern 1: Finnhub 头条为外部抓取文本，消毒后再注入 LLM prompt
                    _hl = n.get("headline", "")[:80]
                    if _ts:
                        _hl = _ts.sanitize_external_text(_hl, max_len=80)
                    lines.append(f"  · [{ts}] {_hl}")
        except Exception:
            pass
...
_SYSTEM_PROMPT_DEFAULT = """你是 Alpha Hive 首席期权策略师，拥有十年衍生品交易经验，专门解读蜂群量化系统的多维信号并生成可操作的深度分析。
...
# pattern 5: persona 从 prompts/options_strategist.md 加载，缺失回退默认常量（行为不变）
SYSTEM_PROMPT = _load_prompt("options_strategist", _SYSTEM_PROMPT_DEFAULT)

# Step 1：分析框架专用系统提示（不写HTML，只做结构化推理）
_STEP1_SYSTEM_DEFAULT = """你是量化信号分析引擎。你的唯一任务是在写报告前完成结构化预分析。
...
# pattern 5: 同上，从 prompts/step1_analysis_engine.md 加载
STEP1_SYSTEM = _load_prompt("step1_analysis_engine", _STEP1_SYSTEM_DEFAULT)
