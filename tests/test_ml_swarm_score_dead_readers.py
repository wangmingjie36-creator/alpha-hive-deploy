...
class TestMissingLedgerIsAccurate:
    ...
    LEDGER_ALIAS = {"market_sentiment": "sentiment", "direction": "direction_encoded",
                    # v0.45.146：报告端用 TrainingData 字段名 crowding_score，
                    # ml_predictor 的 _FEATURE_SOURCES 用短名 crowding
    ...
    #: v0.45.137 三个 + v0.45.141 一个 + v0.45.146 两个 = 7 个。
    #: 名字不写数量：`FIVE` 装了七个就是「列名说谎」，本仓已为此付过账。
