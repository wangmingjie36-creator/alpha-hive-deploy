...
import text_sanitizer as _ts            # pattern 1: 外部文本消毒 + 不可信数据围栏
from prompt_loader import load_prompt as _load_prompt  # pattern 5: persona 从 MD 加载
...
# ==================== 输出 schema 校验 + persona 默认值（pattern 2/5）====================

# llm_service 各 helper 的 LLM 返回 dict 此前「除 JSON 解析外零校验」。
# _coerce_schema 做最小化结构校验：clamp 数值 / enum 兜底 / 截断并消毒字符串 / 截断列表。
# 字符串字段顺带过一遍 sanitize_external_text（pattern 1 纵深防御：LLM 可能回显注入）。

def _coerce_schema(result: Optional[Dict], spec: Dict) -> Optional[Dict]:
    """对 LLM 返回 dict 做最小化结构校验。result 为 None（降级）时原样返回。"""
    ...
    for key, rule in spec.items():
        ...
        if kind == "num":
            ...
            allowed, default = rule[1], rule[2]
            if result.get(key) not in allowed:
                result[key] = default
        elif kind == "str":
            result[key] = _ts.sanitize_external_text(result.get(key), max_len=rule[1])
        elif kind == "list":
            v = result.get(key)
            items = v if isinstance(v, list) else []
            result[key] = [
                _ts.sanitize_external_text(x, max_len=rule[1]) for x in items
            ][: rule[2]]
    return result


# analyze_news_sentiment 的 persona（pattern 5：抽到 prompts/news_sentiment_analyst.md，
# 此常量为文件缺失时的字节级 fallback）
_NEWS_SENTIMENT_SYSTEM = """你是金融新闻情绪分析师。分析给定的新闻标题，判断整体情绪方向。
输出严格 JSON：
...
_NEWS_SENTIMENT_SCHEMA = {
    "sentiment_score": ("num", 0.0, 10.0),
    "sentiment_label": ("enum", {"bullish", "bearish", "neutral"}, "neutral"),
    "key_theme": ("str", 120),
    "reasoning": ("str", 200),
}

_INSIDER_SCHEMA = {
    "intent_score": ("num", 0.0, 10.0),
    "intent_label": ("enum",
                     {"accumulation", "distribution", "planned_exit",
                      "option_exercise", "neutral"},
                     "neutral"),
    "intent_reasoning": ("str", 200),
    "red_flags": ("list", 120, 5),
}

_THESIS_BREAK_SCHEMA = {
    "break_severity": ("enum", {"none", "warning", "critical"}, "none"),
    "break_reason": ("str", 200),
    "new_risk_factors": ("list", 120, 3),
    "recommended_action": ("enum", {"hold", "reduce", "exit"}, "hold"),
}

_DISTILL_SCHEMA = {
    "final_score": ("num", 0.0, 10.0),
    "direction": ("enum", {"bullish", "bearish", "neutral"}, "neutral"),
...
Step 5 — 自我验证：你的判断是否存在确认偏误？如果你错了，最可能的原因是什么？写入 risk_flag""" + _ts.UNTRUSTED_DATA_GUARDRAIL
    ...
    for r in agent_results:
        if r and "error" not in r:
            ...
            agent_summaries.append({
                ...
                # pattern 1: discovery 含上游外部/LLM 文本，消毒后再进 QueenDistiller prompt
                "discovery": _ts.sanitize_external_text(r.get("discovery", ""), max_len=100),
...
请输出 JSON："""
    ...
    # pattern 2: 输出 schema 校验（clamp final_score / confidence，enum 兜底 direction）
    return _coerce_schema(
        call_json(prompt, system=system, max_tokens=800, temperature=0.3),
        _DISTILL_SCHEMA,
    )
...
def analyze_news_sentiment(
    ...
    # pattern 5: persona 从 prompts/news_sentiment_analyst.md 加载，缺失回退内联常量
    # pattern 1: 追加「不可信数据」安全守卫
    system = _load_prompt("news_sentiment_analyst", _NEWS_SENTIMENT_SYSTEM) + _ts.UNTRUSTED_DATA_GUARDRAIL

    # pattern 1: 抓取的标题先消毒（中和注入短语 / 控制字符），再包进不可信数据围栏
    _clean = _ts.sanitize_headlines(headlines, max_len=200, limit=15)
    if not _clean:
        return None
    titles_text = "\n".join(f"- {h}" for h in _clean)
    prompt = f"分析 {ticker} 的以下新闻标题情绪：\n\n{_ts.wrap_untrusted(titles_text, '新闻标题')}\n\n输出 JSON："

    # pattern 2: LLM 返回 dict 做 schema 校验（clamp / enum 兜底 / 截断并消毒字符串）
    return _coerce_schema(
        call_json(prompt, system=system, max_tokens=256, temperature=0.2),
        _NEWS_SENTIMENT_SCHEMA,
    )
...
- red_flags: 值得警惕的信号列表（可为空列表）""" + _ts.UNTRUSTED_DATA_GUARDRAIL

    notable = insider_data.get("notable_trades", [])[:5]
    # pattern 1: SEC 摘要 + 交易明细为外部抓取文本（含内幕人姓名 / 交易描述），
    # 消毒后包进不可信数据围栏
    _summary = _ts.sanitize_external_text(insider_data.get("summary", "无"), max_len=300)
    _notable_txt = _ts.sanitize_external_text(
        json.dumps(notable, ensure_ascii=False), max_len=600, collapse_ws=False)
...
摘要: {_ts.wrap_untrusted(_summary, 'SEC摘要')}
...
输出 JSON："""
    ...
    # pattern 2: 输出 schema 校验
    return _coerce_schema(
        call_json(prompt, system=system, max_tokens=300, temperature=0.2),
        _INSIDER_SCHEMA,
    )
...
- recommended_action: "hold"/"reduce"/"exit\"""" + _ts.UNTRUSTED_DATA_GUARDRAIL

    thesis_text = json.dumps(original_thesis, ensure_ascii=False, default=str)[:400]
    # pattern 1: 最近 7 天新闻为外部抓取文本，消毒后包进不可信数据围栏
    _clean_news = _ts.sanitize_headlines(recent_news or [], max_len=200, limit=10)
    news_text = "\n".join(f"- {h}" for h in _clean_news)
...
## 最近 7 天新闻
{_ts.wrap_untrusted(news_text, '近期新闻') if news_text else "无新闻"}
