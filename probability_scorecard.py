...
_ML_ESTIMATOR_GENERATIONS = [
    ...
    ("2026-09-07", "v0.45.146+v0.45.147",
     "两个并发 session 同日落地，合并为一代（v0.45.146 与 v0.45.147 的合并提交里"
     "把两者的登记合并到本条：v0.45.147 一度并入 09-06 那条，但它落在 09-07；"
     "而 09-06/09-07 两天均无生产扫描 —— predictions 最近业务日 09-04、下次 09-08 ——"
     "所以两条边界之间是空分区，真正要紧的是 **09-08 起适用的这条标签必须把"
     "两次改动都列全**，否则下次分析会以为 09-08 的报告只含 v0.45.146）。"
     "【v0.45.146】服务端最后两个常数特征槽接蜂群真值："
    ...
    ("2026-09-28", "v0.45.334",
     "`final_score` 这个特征的**上游定义**变了（`_prepare_ml_input` 本身一行没动）："
     "v0.45.334 断开 `GexRegimeModifier` 对 rule_score 的直接加减分，`swarm_results.final_score` "
     "自此不含那笔 ±0.8 调整。服务端该特征自 v0.45.140/141 起取 `swarm_results.final_score`，"
     "训练端取 `predictions.final_score`，两端同源 ⇒ 同一槽位的量换了定义。"
     "幅度（生产 `.swarm_results_*.json` 只读反推，见 `ic_rerun_readiness` 同版条目）："
     "09-11~09-22 共 210 行里 133 行 final_score 会变，非零 |Δ| 中位 0.15、最大 0.60。"
     "**没有重放 Δprobability**（没跑模型，不给数）——按上一条的原则「估计量的定义变了就登记」"
     "登记，不等量出输出差异。"
     "⚠️ 如实记一处既有不一致：此前改 final_score 口径的 `_COHORT_HISTORY` 边界"
     "（v0.45.172/176 权重、v0.45.212/228/235 计票与共振等，均晚于 09-07）**一条都没有**在本表登记——"
     "本表迄今只登记过 `_prepare_ml_input` 自身的特征来源改动。本条是第一次按「上游定义变了」登记；"
     "那些旧边界没有补登（补登会把 09-07 起的这一代切成多段，是另一项决定，不搭本版的车）。"
     "日期前提同 `ic_rerun_readiness` 的 v0.45.334 条（原定 09-24 未能赶上，改为 09-28）：本版须在 2026-09-28 14:00 PT 扫描前"
     "推到 main，否则追加一条更正、顺延日期（只追加不改写）"),
]
