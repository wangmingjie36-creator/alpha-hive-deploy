...
def _radar_data(ticker: str, swarm_detail: dict) -> list:
    ...
    if not dim:
        # v0.45.121：文案更正。原写「dim_data_quality 很可能同时为空（整行不渲染），
        # 此时雷达图是唯一能看出无数据的通道」——那是 v0.45.113 当天的实况，
        # 但**次日 v0.45.114 就把它修了**（全缺现在渲染五个「—」）。
        # 一条已经不成立的判断留在运行时日志里，会误导排查的人去别处找原因。
        _log.warning("[%s] 雷达图 dimension_scores 全缺，五维全部画 0。"
                     "同一张卡片的 dim_data_quality 行会显示五个「—」"
                     "（v0.45.114 起），两条通道应当同时报警；只有一条报警"
                     "说明上游 dimension_scores 与 dim_data_quality 不同步。",
                     ticker)
    ...
    # 由扫描侧产出，与这里无关。别再把它当成这里的配套标注。
    #
    # v0.45.121 更正：本段原续写「实测全缺时它五项皆为 None，
    # `_build_dim_dq_html` 会返回空串（整行不渲染），**接不住这种情况**」。
    # 前半句（不是本函数写的）仍然成立；后半句**已被 v0.45.114 推翻**——
    # 全缺现在渲染五个「—」，接得住。
    # （v0.45.114 的 CHANGELOG 曾声称「`_radar_data` 注释里『接不住这种情况』
    #  的说法一并更正」，实际只改了测试文件，这行原封未动 4 处引用里的 1 处。
    #  同 v0.45.75 记的「证伪了，但代码里没改」——**改结论要 grep 它的所有副本，
    #  而且『我已经改了』这句话本身也要核对**。）
    def _d(k):
        v = dim.get(k)
