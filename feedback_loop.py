...
# ─────────────────────────────────────────────
# P2-⑧ 误判模式预警（register_misjudgment_pattern / check_misjudgment_warnings）
# v0.45.215 已撤掉，**勿接回**：写入端每次全量重放历史、非幂等（hits 随运行次数膨胀）；
# walk-forward 无前视检验下预警对后续误判无增量预测力（扣掉标的×方向基线后 −3.6pp，
# CI 跨 0）。库是 pheromone.db 的纯函数，要重新检验跑
# experiments/misjudgment_pattern_walkforward.py，过闸前别再持久化进 thesis_breaks_config。
# ─────────────────────────────────────────────
