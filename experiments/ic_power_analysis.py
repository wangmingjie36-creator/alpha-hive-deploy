...
def permuted_weekly_var(by_day: Dict[str, List[Tuple[float, float]]],
    ...
    **精确值（v0.45.331）**：对固定 x、均匀置换 y，Pearson 相关的置换方差**恒等于
    ...
    三处都不成立：
      ...
      3. 方向也反：随机分数给的数更**小**。换成置换后 4/5 维照样钳 0——这次改动
         没有翻转任何结论。
    仍用置换（标准的条件置换检验，结果正确）。不复用 noise_floor 的真正理由：它只用
    **一个**基准因子的骨架（base_key），方差分解要每个维度自己的天集合；且它输出
    |IC| 分位数而非周度方差。noise_floor 对它自己的用途是对的，**不要去"修"它**。
    """
