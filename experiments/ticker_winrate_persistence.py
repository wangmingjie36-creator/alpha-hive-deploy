...
────────────────────────────────────────────────────────
  ...
  另做前后半段分割检验（2026-05-03 为界，10 只样本≥15 的标的）：
    前后半胜率 Spearman = -0.139（负相关）
    AMZN 85.7% → 25.0%；QCOM 61.9% → 43.3%；META 29.2% → 56.2%
    （v0.45.25 更正：原文的 -0.273 / AMZN 83.9%→27.3% / META 38.7%→58.8%
     是 v0.45.9 ambiguous 修正**之前**的跑批，已与本脚本当前输出不符。
     现值经 2026-08-26 重跑核实，与 CHANGELOG v0.45.12 条目一致。
     其余各组数字——触发/未触发组与五分层——两次跑批完全相同。）
