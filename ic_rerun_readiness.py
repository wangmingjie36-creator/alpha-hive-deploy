...
"⚠️ 与同日落地的 v0.45.191（边界 2026-09-10）的交互：那条为了不新开空分区刻意与 v0.45.176 同日、并据实测记「作废 0 条」。本条比它晚一天，**按只取最后一条的判据，09-10 那 30 条样本会被本条作废** —— 不是它算错了，是本条改的是数据源、09-10 的分确实是用旧 GEX 口径打的。记在这里免得下一个人以为两条矛盾。"),
    ...
    ("2026-09-13", "v0.45.238",
     ...
     "按 predictions.created_at 与快照时间戳比对推断，吃到前一会话期权数据的预测日为 "
     "07-08（1 只）、07-22、07-23、07-24、08-14、09-03、09-09，约 119 行 —— **均早于本边界、已被此前各代作废**，"
     "但全历史口径的分析（不按世代切的 IC / 期权列诊断）仍会读到它们，**历史快照与台账未改写**。 "
     "收盘后跑的生产扫描里会话日期 == 太平洋日期，文件名与输出逐位不变，只有跨午夜/盘前的日子会不同。 "
     "**与 v0.45.212 / v0.45.228 / v0.45.235 共用 2026-09-13 标签**：该边界之后到本版落地 predictions 0 条 ⇒ **作废 0 条**。"),
]
