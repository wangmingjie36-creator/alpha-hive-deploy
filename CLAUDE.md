...
### 并发开工必须先占号（2026-08-27 起）

多个 session 同时开工时，**开工第一件事就是占号**，不要等完工再编号：

1. `git fetch origin` 后看 `origin/main` 的 `CHANGELOG.md` 取下一个可用号
2. 在顶部插入占位条目：
   `## [0.45.NN] — YYYY-MM-DD — 占位（进行中：一句话说明在做什么）`
3. **立刻单独提交并推送到 main**（只含这一行改动，不带代码）
4. 完工时把占位标题替换成真标题，正文补齐

⚠️ **第 3 步不能省。** 占位写在自己的 worktree 里，别的 session 看不见 ——
它们读的是主 checkout / `origin/main`。不推送的占号等于没占。

**为什么值得**：2026-08-26~27 两条线并行，`0.45.36–0.45.40` 被双方各用了一遍、
`0.45.47` 与 `0.45.48` 又各撞一次，前后返工三轮（一方整批后移五个号、
另一方让号两次）。占号只要几十秒，撞号的代价是改 CHANGELOG 标题 + 改提交信息
+ 重跑核对。
...
同一形状在四天内出现四次，横跨三个 session 与一个自动流程，**表层原因毫无关系、机制完全相同**：

| 版本 | 失败点 | 谁本该知道 | 实际 |
|---|---|---|---|
| v0.45.91 | 陈旧信号被压成 `None` | 补抓分支 | `pass` 吞掉 → 死代码从未触发 |
| v0.45.119 | 解释器不存在，抛 `FileNotFoundError` | `pytest.skip` 的 returncode 检查 | 守卫只判 returncode，接不住抛 → 本该跳过变成 7 个 ERROR |
| v0.45.121 | 脚本 `assert` 失败 | 相邻行的 `git push` | `&&` 链断在前一步无人察觉 → 冲突标记进了 main |
| v0.45.124 | `_fetch_cboe_payload` 真出网并失败 | `tests/test_quote_set.py` 的断言 | `except Exception` 静默降级 → 42 项全绿，代价只是慢 220 秒 |

两条可执行的推论：

1. **写 `except` / `or 默认值` / `if 取不到: continue` 时，问一句「谁会红？」**
   若答案是「没人」，就补一个会红的观测点（断言、计数、探针），别指望下次自己想起来。
   与 v0.45.114「跳过缺失项＝把缺失渲染成不存在」同源。
2. **写守卫先问「失败时是返回还是抛」，两条路径各堵一次。**
   只判返回值的守卫接不住 `raise`；只判异常的接不住 `return None`。

⚠️ 特别地，**「这个测试文件是离线的」是一句需要被执行的断言，不是注释。**
v0.45.124 的 `test_quote_set.py` docstring 写着「不碰网络」，实测 5 个测试函数
每个打 6 次外网——因为出网失败被降级路径吞了，没有任何断言依赖它。
...
v0.45.157 实测：`TestSPYBenchmarkUnavailable` 三条挂着「缺生产 `pheromone.db` 就 skip」，
而 `pheromone.db` 命中 `.gitignore` 的 `*.db` ⇒ 干净检出与 CI 上恒 skip；多数 worktree 里
那份库是**别的测试写出来的空壳**（0 行 predictions），卡在第二层 skip；只有主 checkout
两层都过、真跑，然后打 yahoo 外网被离线闸判红。
⇒ **唯一会执行它的地方，正是唯一会失败的地方**；三条测试的断言从未被求值过。

三条配套判据：

1. **依赖若由测试自己构造，守卫就该是断言不是 skip。** 构造坏了必须变红。
   夹具「接上了没」要**正面核对**（读到的条数/键集合等于夹具的），不能只断言「没读生产库」。
2. **同样的条件性，写在 marker 上可见，写在 skip 里不可见。**
   `-m integration` 由 addopts 显式排除、由命令行显式选中；skip 只在 `-rs` 时吐一行，
   默认输出里它和 PASSED 一样是个点。真需要外部系统的测试标 marker，别写 skip。
3. **数一数还有多少条挂在同一个 X 上。** 同次实测另有 17 条恒 skip 在三个文件里。
   但**别一刀切**：核对*生产数据本身*有没有退化的测试（如 `test_distribution_invariants`）
   换夹具就测不到它要测的东西，那类该标 marker 而不是补桩。

与上一节及「这个失败，下游怎么知道？」同源：`continue` 把「这项缺了」渲染成「这项不存在」，
`skip` 把「这条没验」渲染成「这条没问题」。都答不出那句「**谁会红？**」。

⚠️ 附带一条取证教训：生产 `pheromone.db` 是 **WAL 模式**，*任何*连接（含 `mode=ro` 只读探针）
都会就地造出 `-wal`/`-shm` 边车。核对「测试写没写生产库」时别拿边车当证据，
要比 **sha256 + mtime**；且对照实验的每一格必须**处置完全相同**，
否则差异来自你的探针而不是被测代码（v0.45.157 差点据此记下一条假回归）。
